@extends('layouts.guest')

@section('content')
   <div class="container my-5">
      {{-- <a href="{{ route('login') }}" class="bg-coklat text-decoration-none rounded text-light px-4 py-2 border-0">Login</a> --}}
      <div class="row justify-content-center my-3">
         <div class="col-10 col-md-8 mx-auto">
            <div class="text-center">
               <h1>Browse By Contributors</h1>
            </div>

            <div class="my-2 text-center mx-auto">
              <div class="d-flex align-items-center justify-content-center gap-2">
                <input id="contributor-name-input" name="query" class="form-control" type="text" placeholder="" aria-label="Search input example">
                <button id="fetch-contributor-data" class="btn btn-primary">Browse</button>
              </div>
            </div>

            <div id="spinner-container" class="text-center my-3" style="display: none;">
              <div class="spinner-border" role="status">
                <span class="visually-hidden">Loading...</span>
              </div>
            </div>

            <div id="eprints-container" class="d-flex flex-column mt-4">
              <!-- Eprints data  -->
            </div>
         </div>
      </div>
   </div>

   @push('scripts')
    <script>
      $('#fetch-contributor-data').click(function() {
        var contributor_name = $('#contributor-name-input').val();
        if (contributor_name) {
            $('#spinner-container').show();
            
            $.ajax({
                url: '{{ route("browse.document.author") }}',
                method: 'GET',
                data: { author_name: contributor_name },
                success: function(response) {
                    var container = $('#eprints-container');
                    container.empty(); 

                    if (response.length > 0) {
                        $.each(response, function(index, eprint) {
                          var fileUrl = `{{ route('file.open', ':filename') }}`.replace(':filename', eprint.file);
                          var item = `
                            <div class="my-2">
                              <a href="${fileUrl}" target="_blank" class="text-decoration-none">${eprint.title}</a>
                            </div>
                          `;
                            container.append(item);
                        });
                    } else {
                        container.html(`
                        <div class="alert alert-danger" role="alert">
                          No documents found for the contributor "${contributor_name}".
                        </div>`);
                    }
                },
                error: function() {
                    swal("Error!", "Error fetching data", "error");
                },
                complete: function() {
                    $('#spinner-container').hide();
                }
            });
        } else {
            swal("Heyy!", "Please enter an contributor name!", "warning");
        }
    });

    </script>
   @endpush
@stop
